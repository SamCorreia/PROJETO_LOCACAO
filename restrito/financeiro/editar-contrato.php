<?php
	...
	$stm = $con->query("SELECT *, (select razao_social from localferramentas_cadastroempres where id = contrato_dados.empresa) as empresa_nome, (select cnpj from localferramentas_cadastroempres where id = contrato_dados.empresa) as cnpj FROM contrato_dados WHERE id = '$id_contrato'");
	while($b = $stm->fetch()){ 
		extract ($b); 
	}
?>
